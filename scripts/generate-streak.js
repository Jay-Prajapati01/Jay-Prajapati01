...
function buildStreakSVG(stats) {
  ...
  <text x="${c1Mid}" y="${subY}" text-anchor="middle" font-size="10" fill="${COLORS.dim}">${stats.lastContributionDate ? 'Last: ' + escapeXml(formatDate(stats.lastContributionDate)) : '—'}</text>`;
